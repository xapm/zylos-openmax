...
- **`not_authorized`** (case B — not connected to you): send the authorize card with `conn.auth_notice {conversationId, apps:["<slug>"]}` — it shows the user a button that opens that app's connect dialog directly. Alongside it, one short plain sentence is enough (e.g. 「{app} 还没有授权，点下面的卡片去授权，授权后在对话里打开它就能用了。」); do not repeat the card's contents at length. If `conn.auth_notice` fails (e.g. the app isn't in the catalog), fall back to saying plainly that {app} isn't connected yet and pointing them to the **Connections** page (`/connections`). Do not invent another mechanism.
